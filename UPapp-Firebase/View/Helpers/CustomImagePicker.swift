...
fileprivate struct CustomImagePicker<Content: View>: View {
    ...
    @State private var selectedCropType: Crop = .circle(.init(width: 300, height: 300))
...
struct CropView: View{
    ...
    func ImageView(_ hideGrids: Bool = false)->some View{
        ...
        .frame(cropSize)
        .cornerRadius(crop.name() == "Circle" ? cropSize.height / 2 : 0)
    }
...
struct CustomImagePicker_Previews: PreviewProvider {
    static var previews: some View {
        CropView(crop: .square(.zero), image: UIImage(named: "Sample Pic")) { _, _ in
